/* HAKIMAIL — LocalStorage edition with passwords */
...
const btnViewPasswords = document.getElementById('btn-view-passwords');
const allPasswordsDiv = document.getElementById('all-passwords');
...
function loadPasswords(){
  ...
  if(!p){
    p = {};
    DEPARTMENTS.forEach(d=>{ p[d] = d==="admin" ? "admin123" : "1234"; });
    localStorage.setItem("Passwords", JSON.stringify(p));
  }
  return p;
}
function savePasswords(p){ localStorage.setItem("Passwords", JSON.stringify(p)); }

// ---------- Initialization ----------
function init(){ populateLogin(); }
...
// ---------- Change password (για όλους εκτός admin) ----------
btnChangePass.onclick = () => {
  if(currentUser === "admin"){
    alert("Ο admin δεν αλλάζει τον δικό του κωδικό από εδώ. Δες 'Προβολή Κωδικών'.");
    return;
  }
  const passwords = loadPasswords();
  const newPass = prompt("Δώσε νέο κωδικό:");
  if(newPass && newPass.trim().length>=3){
    passwords[currentUser] = newPass.trim();
    savePasswords(passwords);
    alert("Ο κωδικός άλλαξε!");
  }
};

// ---------- View all passwords (admin only with edit) ----------
btnViewPasswords.onclick = () => {
  if(currentUser !== "admin"){
    alert("Μόνο ο admin μπορεί να δει τους κωδικούς.");
    return;
  }
  const passwords = loadPasswords();
  if(allPasswordsDiv.classList.contains("hidden")){
    let html = `
      <table style="width:100%;border-collapse:collapse;">
        <tr style="background:#f0f0f0;">
          <th style="padding:6px;border:1px solid #ddd;">Τμήμα</th>
          <th style="padding:6px;border:1px solid #ddd;">Κωδικός</th>
          <th style="padding:6px;border:1px solid #ddd;">Ενέργεια</th>
        </tr>
    `;
    Object.entries(passwords).forEach(([u,p])=>{
      html += `
        <tr>
          <td style="padding:6px;border:1px solid #ddd;">${u}</td>
          <td style="padding:6px;border:1px solid #ddd;">
            <input type="text" id="pass-${u}" value="${p}" style="width:100%;padding:4px;">
          </td>
          <td style="padding:6px;border:1px solid #ddd;">
            <button onclick="updatePassword('${u}')">Αποθήκευση</button>
          </td>
        </tr>
      `;
    });
    html += `</table>`;
    allPasswordsDiv.innerHTML = html;
    allPasswordsDiv.classList.remove("hidden");
  } else {
    allPasswordsDiv.classList.add("hidden");
  }
};

// ---------- Admin: update password of specific user ----------
function updatePassword(user){
  const passwords = loadPasswords();
  const newPass = document.getElementById("pass-"+user).value.trim();
  if(newPass.length < 3){
    alert("Ο κωδικός πρέπει να έχει τουλάχιστον 3 χαρακτήρες.");
    return;
  }
  passwords[user] = newPass;
  savePasswords(passwords);
  alert("Ο κωδικός του χρήστη '"+user+"' άλλαξε!");
}
...
function openDoc(data, type){
  ...
  if (type && type.startsWith('image/')) {
    ...
    modalContent.innerHTML = `<div class="meta">Δεν προβάλλεται · <a href="${data}" download>Κατέβασέ το</a></div>`;
  }
