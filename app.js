/* HAKIMAIL — LocalStorage edition με αλλαγή κωδικών, αυτόματο καθάρισμα & admin ρυθμίσεις */
...
const passwordsPage = document.getElementById('passwords-page');
...
const btnOpenPasswords = document.getElementById('btn-open-passwords');
...
const btnBackFiles = document.getElementById('btn-back-files');
...
function cleanupOldDocs(){
  let docs = loadLS("Documents");
  const now = new Date();
  docs = docs.filter(d => {
    const dt = new Date(d.date);
    return (now - dt) < 24*60*60*1000; // 1 μέρα
  });
...
btnLogin.onclick = () => {
  ...
  if (!(dep in users) || users[dep] !== pass) {
    alert('Λάθος κωδικός');
    return;
  }
...
btnLogout.onclick = () => {
  ...
  passwordsPage.classList.add('hidden');
...
btnChangePass.onclick = () => {
  const users = getUsers();
  if(currentUser === "admin"){
    alert("Οι κωδικοί φαίνονται από το μενού Ρυθμίσεις → Προβολή Κωδικών");
  } else {
    const newPass = prompt("Δώσε νέο κωδικό για " + currentUser);
    if(newPass && newPass.trim().length >= 3){
      users[currentUser] = newPass.trim();
      saveUsers(users);
      alert("Ο κωδικός άλλαξε με επιτυχία!");
    } else {
      alert("Άκυρος κωδικός.");
    }
  }
};
...
uploadForm.onsubmit = (e) => {
  e.preventDefault();
  if (!currentUser) { alert("Δεν είσαι συνδεδεμένος!"); return; }
  ...
  reader.onload = (ev) => {
    ...
    const doc = {
      ...
      from: currentUser,      // πάντα το login user
      to: toSelect.value,
      note: msgInput.value || '',
      date: new Date().toISOString(),  // ISO για σωστή σύγκριση
...
function loadDocuments(){
  ...
  console.log("DEBUG - inbox:", inbox, "outbox:", outbox);
...
function renderItem(d){
  ...
  const meta = `${escapeHtml(d.from)} → ${escapeHtml(d.to)} · ${new Date(d.date).toLocaleString()}`;
...
// ---------- Passwords page ----------
btnOpenPasswords.onclick = () => {
  if(currentUser !== 'admin') return;
  dashboard.classList.add('hidden');
  passwordsPage.classList.remove('hidden');
  showPasswordsPage();
};
btnBackPasswords.onclick = () => {
  passwordsPage.classList.add('hidden');
  dashboard.classList.remove('hidden');
};
function showPasswordsPage(){
  const users = getUsers();
  passwordsList.innerHTML = Object.entries(users).map(([u,p]) =>
    `<div class="item"><strong>${escapeHtml(u)}</strong> <div class="meta">Κωδικός: ${escapeHtml(p)}</div></div>`
  ).join('');
}

// ---------- Files page ----------
btnOpenFiles.onclick = () => {
  if(currentUser !== 'admin') return;
  dashboard.classList.add('hidden');
  filesPage.classList.remove('hidden');
  showFilesPage();
};
btnBackFiles.onclick = () => {
  filesPage.classList.add('hidden');
  dashboard.classList.remove('hidden');
};
function showFilesPage(){
  const docs = loadLS("Documents").sort((a,b)=>new Date(b.date)-new Date(a.date));
  filesList.innerHTML = docs.length ? docs.map(d => `
    <div class="item">
      <strong>${escapeHtml(d.name)}</strong>
      <div class="meta">${escapeHtml(d.from)} → ${escapeHtml(d.to)} · ${new Date(d.date).toLocaleString()}</div>
    </div>`).join('') : '<div class="meta">Κανένα αρχείο</div>';
}
